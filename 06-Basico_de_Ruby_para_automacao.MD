...
==  # igual
!=  # diferente
if not, unless, if !variavel # Operador lógico que representa a negação (inverso de true) da variável atual. Ou seja somente executa se o resultado for false. O uso do simbolo de afirmação ***"!"*** para ter a condição negativa é a mais comum no ruby, eu prefiro o uso do not, acho mais claro de se entender.
